...
--Modified by Brett Hixon 5/10/2025
-------------------------------------------------

ALTER   PROC [dbo].[AddReview](
	@UserID INT,
    @GameID INT,
    @Star INT,
    @Content VARCHAR(MAX),
...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM Reviews r WHERE r.GameID = @GameID AND r.UserID = @UserID)
		THROW 51004, 'This user has already left a review on this game.', 1
